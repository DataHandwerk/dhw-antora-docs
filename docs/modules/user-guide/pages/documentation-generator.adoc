...
* https://gitlab.com/datahandwerk/dhw-antora-docs[] contains manually created content like architecture, manual
* https://gitlab.com/datahandwerk/dhw-antora-sqldb[] contains automatically created content, created using xref:sqldb:docs.usp_antoraexport.adoc[]

steps

* create a new git repositories per database, for example https://gitlab.com/mygroup/dhw-antora-sqldb[]
** you can create and use local repositories
** you can combine different documentation sources in one documentation
*** each dwh database has its own repository database
...
You can document each database as a separate component or as different modules of a common component. Each component has its own navigation bar, switching between components is done with a component selector, by default in the lower left corner.
* *at least one commit is required to enable Antora to use any repository*
* create required folder structure
** copy the template for an Antora-component-folder from `DataHandwerk-toolkit-mssql\antora-template\docs` (including all content) into your target repository 
...
[source]
----
xcopy D:\Repos\gitlab\DataHandwerk\DataHandwerk-toolkit-mssql\antora-template\docs D:\Repos\aaa\bbb\MyDataBase_sqldb\docs /t /e
----
+
** copy the folder `docs\modules\template` into a folder `docs\modules\sqldb`, if you use the default AntoraModule name "sqldb"
** if you don't use the default AntoraModule "sqldb" then rename the folder to match your 'AntoraModule' parameter settings in xref:dhw:sqldb:config.parameter.adoc[]
** check and change the Antora navigation files for each module
*** `docs\modules\sqldb\nav.adoc`
*** example file content (Remove the `/` at the beginning of the line before the `include`.)
+
====
....
...
....
====
* setup your 'AntoraComponentFolder' parameter settings in xref:dhw:sqldb:config.parameter.adoc[]
====
[source,sql]
----
...
----
====
* set your 'AntoraDeleteFilesInModuleFolders' parameter settings in xref:dhw:sqldb:config.parameter.adoc[] to '1' +
...
+
In DataHandwerk We use two (2) docs source repositories, and we need the `antora.yml` in each source repository. The `title` should be contained only in one `antora.yml`.
+
...
DataHandwerk `dhw-antora-sqldb` contains only automatically generated documentation. Additional manual documentation is located in an additional repository `dhw-antora-docs`
...
content:
  sources:
  - url: D:\Repos\gitlab\DataHandwerk\DataHandwerk-docs
    branches: HEAD
    start_path: docs
  - url: D:\Repos\gitlab\DataHandwerk\sqldb-docs
    branches: HEAD
    start_path: docs
asciidoc:
  attributes:
