= SQL Server bug in sys-views for references
...
The reason is a SQL Server bug.
...
We can't solve the SQL Server bug, we can only avoid to get references for some problematic objects.
