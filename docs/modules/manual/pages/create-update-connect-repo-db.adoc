...
[source,sql]
----
include::sqldb:partial$repo.usp_connect_database.adoc[tag=example]
----
====


IMPORTANT: using *publish* or *dacpac* deployment to update the repo database will reset synonyms. +
reconnect the repo database to the right dwh database after using publish or dacpac deployment by executing `[repo].[usp_connect_database]`
// --
...
// include::sqldb:example$/dbdhw/repo/Stored Procedures/usp_connect_database.sql[tag=example]

xref:sqldb:repo.usp_connect_database.adoc[]
