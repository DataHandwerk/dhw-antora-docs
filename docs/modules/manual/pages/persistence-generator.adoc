...
The xref:manual:persistence-generator.adoc[] is used to create the persistence procedure code. Depending on changed settings in xref:sqldb:repo.RepoObject_persistence.adoc[] the generated code will change. After changing settings it is required to execute xref:sqldb:repo.usp_main.adoc[]
...
[WARNING]
--
It can be very difficult to change the design of graph tables in SQL Server, especially when https://docs.microsoft.com/en-us/sql/relational-databases/tables/graph-edge-constraints?view=sql-server-ver15[Edge Constraints] are used. The code created by SSMS or by Visual Studio code is then unusable and does not work. I Azure Data Studio it is even worse.
--
