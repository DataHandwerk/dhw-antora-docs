...
IMPORTANT: The procedure xref:sqldb:repo.usp_persistence_insert_update.adoc[] is not called automatically. If a persistence is to be created or changed, this must be done manually!

IMPORTANT: Before you execute the procedure, check, if the source should have a real or virtual PK. Some persistence actions are not available, if no PK is defined. Execute xref:sqldb:repo.usp_Index_virtual_InsertUpdate[] to define virtual PK, whenever this is possible.

====
[source,sql]
----
include::sqldb:partial$repo.usp_persistence_insert_update.adoc[tag=example1]
----
====
...
The code is available in xref:sqldb:repo.GeneratorUsp_SqlUsp.adoc#column-SqlUsp[repo.GeneratorUsp_SqlUsp.SqlUsp]
...
* then EXEC repo.[usp_persistence_insert_update]
